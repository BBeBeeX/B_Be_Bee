...
- fix(ios): share from other apps to b_be_bee doesn't work in iOS 18 (@Tienisto)
...
- feat(macos): include b_be_bee as a share target in the share menu (@ShlomoCode)
...
- feat(macos): b_be_bee installed via dmg installer is sandboxed (@Tienisto)
...
- fix(windows): b_be_bee window is invisible at app start (@Tienisto)
...
- feat: share via browser link (for non-b_be_bee users)
...
- feat: implement b_be_bee protocol v2 with v1 fallback
