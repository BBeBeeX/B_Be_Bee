...
class ShaderWidget extends StatefulWidget {
  ...
  @override
  State<ShaderWidget> createState() => _ShaderState();
}

class _ShaderState extends State<ShaderWidget> with SingleTickerProviderStateMixin {
  ...
  Uint8List createBmpFromAudioData() {
    print(widget.data);
    if (widget.data.length < 512) return Uint8List(0);
    ...
    for (var x = 0; x < cols; x++) {
      // fill FFT values
      final fft =
          ((widget.data[x + minBinIndex] * widget.params.audioScale).clamp(0.0, 1.0) *
                  255)
              .toInt();
      img[x * 4] = fft; // R
      img[x * 4 + 1] = 0; // G
      ...
      // fill wave values
      final wave = ((((widget.data[x + minBinIndex + 256] * widget.params.audioScale)
                          .clamp(-1.0, 1.0) +
                      1.0) /
                  2.0) *
              255)
