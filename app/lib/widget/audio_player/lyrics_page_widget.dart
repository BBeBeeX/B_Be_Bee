...
class LyricsPageWidget extends ConsumerWidget {
  ...
  const LyricsPageWidget({super.key, this.fontColor = Colors.white});

  TextStyle getLyricTextStyle(int index, int curLine, int lyricsLength) {
    bool isHighlighted = index == curLine - 1;
    bool isFaded = (index <= curLine - 3 || index >= curLine + 4) &&
            (curLine > 3 && curLine < lyricsLength - 3) ||
        (index == 7 && curLine <= 3) ||
        (index == lyricsLength - 7 && curLine >= lyricsLength - 3);

    return TextStyle(
      fontSize: isHighlighted ? 30 : (isFaded ? 15 : 20),
      color: fontColor.withOpacity(isHighlighted ? 1.0 : (isFaded ? 0.3 : 0.6)),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return StreamBuilder<Duration>(
      ...
      builder: (context, snapshot) {
        ...
        int indexLine =
            lyrics.indexWhere((lyric) => lyric.startTime > position);
            ...
            GestureDetector(
              ...
              onVerticalDragUpdate: (e) {
                ...
                final newOffset = (scrollController.offset - e.delta.dy).clamp(
                  0.0,
                  (lyrics.length - LyricsController.visibleLines) *
                      lyricsState.itemHeight,
                );
              ...
              child: Center(
                child: SizedBox(
                  height:
                      lyricsState.itemHeight * LyricsController.visibleLines,
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Container(
                        ...
                        child: AnimatedDefaultTextStyle(
                          ...
                          style:
                              getLyricTextStyle(index, curLine, lyrics.length),
                          child: Column(
                            children: [
