...
final collectsPlaylistPageProvider = StateNotifierProvider.family<CollectsPlaylistPageController, CollectsPageVm, String>(
  (ref, params) => CollectsPlaylistPageController(ref, params),
...
class CollectsPlaylistPageController extends StateNotifier<CollectsPageVm> {
  ...
  CollectsPlaylistPageController(this.ref, this.collectPlaylistId) : super(const CollectsPageVm());

  List<AudioInfo> _getSortedMedias(List<AudioInfo> medias, SortMethodEnum method) {
    ...
    switch (method) {
      ...
      case SortMethodEnum.artistAZ:
        sorted.sort((a, b) => (a.upper.name ?? '').compareTo(b.upper.name ?? ''));
        break;
  ...
  Future<void> loadData(CollectPlaylist collectPlaylist) async {
    ...
    try {
      if(collectPlaylist.songs == null){
        switch (collectPlaylist.collectCurrentType) {
          case CollectTypeEnum.biliCollect:
            final res  = await BiliCollectsApi.getAllCollectsResource(int.parse(collectPlaylist.onlineId!));
          ...
          case CollectTypeEnum.biliSeason:
            if(collectPlaylist.songs == null){
              final res = await BiliCollectsApi.getSeasons(int.parse(collectPlaylist.onlineId!));
              collectPlaylist = collectPlaylist.copyWith(
                  title: res.title,
                  songs: res.songs,
                  songIds: res.songs!.map((media) => media.id).toList(),
          ...
          case CollectTypeEnum.local:
            ...
            ref.listen(collectsProvider, (previous, next) {
              final playlist = next.playlists.firstWhere(
                    (p) => p.id == collectPlaylist.id,
              );
              ...
              if(ref.read(settingsProvider).autoSyncToLocal){
                try {
                  await ref.read(collectsProvider.notifier)
                      .syncPlaylist(collectPlaylist.id ??'',collectPlaylist.songs!);

                  await Future.microtask(() {
                    container.read(commonLoggerProvider.notifier).addLog('collects playlist page synchronized: ${collectPlaylist.id} ${collectPlaylist.collectCurrentType} ${collectPlaylist.songIds}');
                  });

                } catch (e) {
                  await ToastUtil.show('${t.general.synchronizedFailed}: ${e.toString()}');
                }
              }
            }
          },
      ...
      final sortedMedias = _getSortedMedias(
  ...
  void setSortMethod(SortMethodEnum method) {
    if (state.collectPlaylist == null || state.collectPlaylist?.songs == null) return;

    final sortedMedias = _getSortedMedias(
      state.collectPlaylist!.songs!,
      method
    );
  ...
  Future<void> reorderSongs(int oldIndex, int newIndex) async {
    ...
    final medias = List<AudioInfo>.from(state.sortedMedias ?? state.collectPlaylist!.songs!);
    ...
    if (state.collectPlaylist?.collectCurrentType == CollectTypeEnum.local ) {
      final playlist = HiveHelper.getCollectsPlaylist(state.collectPlaylist?.id ??'');
      if (playlist != null) {
