...
final biliPasswordLoginProvider = StateNotifierProvider<BiliPasswordLoginNotifier, BiliPasswordLoginVm>((ref) {
...
class BiliPasswordLoginNotifier extends StateNotifier<BiliPasswordLoginVm> {
  ...
  BiliPasswordLoginNotifier(this.ref) : super(BiliPasswordLoginVm(
    ...
    try {
      await startBiliCaptcha(
        onSuccess: (captchaResult) async {
          ...
          try {
            final passwordLoginKeyHash = await BiliLoginApi.requestPasswordLoginKeyHash();
            if (passwordLoginKeyHash.code != 0) {
              await ToastUtil.show( t.controller.bili.login.networkErrorPubError);
              return;
            }

            final PostPasswordLoginResponse passwordLoginResult = await BiliLoginApi.postPasswordLogin(
                captchaResult,
                passwordLoginKeyHash,
                account,
                password
            );

            if (passwordLoginResult.code != 0) {
              await ToastUtil.show(t.controller.bili.login.passwordLoginErrorForMessage(message: passwordLoginResult.message));
              return;
            }
            ...
            if (passwordLoginResult.data.status == 0 || hasLogin) {
              ...
              if(passwordLoginResult.data.refresh_token.isNotEmpty){
                ...
                await HiveHelper.setBiliRefreshToken(passwordLoginResult.data.refresh_token);
              }
              ...
              ref.read(biliMyLoginTabProvider.notifier).switchToTab(LoginModeEnum.sms);
            } else {
              await ToastUtil.show(t.controller.bili.login.passwordLoginErrorForMessage(message: passwordLoginResult.message),notShow:',code:${passwordLoginResult.data.status}');
            }
          } catch (e) {
