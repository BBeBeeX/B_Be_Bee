...
class BiliUserApi {
  ...
  static Future<UserCardResponseData> getCollects2Resource(String uid) async {
    ...
    final userCardResponse = await RhttpUtils().get(
        ApiConstants.usercard,
        query: {
          'uids': uid.toString(),
        }
    );

    return UserCardResponse.fromJson(userCardResponse.bodyToJson).data?.first ?? UserCardResponseData() ;
  }
