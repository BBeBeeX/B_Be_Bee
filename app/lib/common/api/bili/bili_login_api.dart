...
abstract class BiliLoginApi {
  ...
  static Future<PasswordLoginKeyHashResponse>
    ...
    var responsePasswordPublicKeyHash = await RhttpUtils().get(
      ApiConstants.passwordPublicKeyHash,
    );
    return PasswordLoginKeyHashResponse.fromJson(responsePasswordPublicKeyHash.bodyToJson);
  }

  ///获取人机测试所需要的数据
  static Future<CaptchaDataResponse> requestCaptchaData() async {
    var responseCaptcha = await RhttpUtils().get(
  ...
  static Future<PostSmsRequireResponse> postSendSmsToPhone(int cid, int tel,
    ...
    var responseSmsCode = await RhttpUtils().post(ApiConstants.smsCode,
        query: {
          'cid': cid.toString(),
          'tel': tel.toString(),
          'source': 'main_web',
  ...
  static Future<PostSmsLoginResponse> smsLogin(
    ...
    var responseSmsLogin = await RhttpUtils().post(ApiConstants.smsLogin,
        query: {
          'cid': cid.toString(),
          'tel': tel.toString(),
          'code': code.toString(),
  ...
  static Future<PostPasswordLoginResponse> postPasswordLogin(
    ...
    //先获取cookie
    await RhttpUtils().get(ApiConstants.bilibiliBase,);
    //加密密码
    ...
    String passwordEncryptyed = Encrypter(RSA(publicKey: publicKey))
        ...
        .base64;
    var responsePasswordLogin = await RhttpUtils().post(ApiConstants.passwordLogin,
        query: {
          'username': username,
  ...
  static Future<LoginUserInfoResponse> _requestLoginUserInfo() async {
    var responseUserInfo = await RhttpUtils().get(
  ...
  static Future<LoginQRcodeInfo> getQRcode() async {
    var responseQrcodeGenerate = await RhttpUtils().get(ApiConstants.qrcodeGenerate);
  ...
  static Future<PostQrcodeLoginData> checkQRcodeLogin(
    ...
    final  responseQrcodeLogin = await RhttpUtils().get(ApiConstants.qrcodeLogin,
        query: {'qrcode_key': qrcodeKey});

    final PostQrcodeLogin postQrcodeLogin = PostQrcodeLogin.fromJson(responseQrcodeLogin.bodyToJson);

    return postQrcodeLogin.code == 0 ? postQrcodeLogin.data:PostQrcodeLoginData();
  }
  ...
  static Future<int> _isNeedToRefresh() async {
    var responseCookieInfoJSON = await RhttpUtils().get(
      ApiConstants.cookieInfo,
    );

    try {
      GetCookieInfo responseCookieInfo = GetCookieInfo.fromJson(responseCookieInfoJSON.bodyToJson);
  ...
  static Future<String> _getRefreshCSRF(String correspondPath) async{
    var responseRefreshCSRF = await RhttpUtils().get(
  ...
  static Future<void> _postRefreshCookie(String refreshToken,String refreshCsrf) async{
    ...
    final postRefreshToken = await RhttpUtils().post(
      ApiConstants.postRefreshCookiePath,
      query: formData,
      headers: HttpHeaders.map({
        HttpHeaderName.contentType: Headers.formUrlEncodedContentType,
      }));
    ...
    if(postRefreshToken.bodyToJson!.refresh_token!.isNotEmpty){
      await HiveHelper.setBiliRefreshToken(postRefreshToken.bodyToJson!.refresh_token);
    }else{
      print('refresh token error :');
      print('PostRefreshToken code${postRefreshToken.statusCode}');
      print('PostRefreshToken message${postRefreshToken.body}');
      print('PostRefreshTokenData status${postRefreshToken.bodyToJson!.status}');
      print('PostRefreshTokenData message' + postRefreshToken.bodyToJson!.message!);
    }
  }
}
