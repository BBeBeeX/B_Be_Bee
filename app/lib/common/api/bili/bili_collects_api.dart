...
abstract class BiliCollectsApi {
  ...
  //获取当前用户创建的所有收藏夹信息
  static Future<List<CollectsDataDto>> getAllCreatedCollects(String userId) async {
  ...
  //获取当前用户创建的所有收藏夹信息
  static Future<CollectsListAllDto> getAllCreatedAllCollects(String userId) async {
    // final mid = await CookieUtils.getUserId();

    final createdListAll = await RhttpUtils().get(
      ApiConstants.createdListAll,
      query: {'up_mid': userId}
    );

    return CreatedListAll.fromJson(createdListAll.bodyToJson).data ?? CollectsListAllDto(count: 0);
  }

  //获取收藏夹封面信息
  static Future<CollectsDataDto> getCollectsInfo(int collectId) async {

    final collectsInfo = await RhttpUtils().get(
      ApiConstants.resourceInfo,
      query:{'media_id': collectId.toString()}
    );
    return ResponseCollectsInfo.fromJson(collectsInfo.bodyToJson).data ?? CollectsDataDto(cover: Constants.bili_video_default_cover);
  }
  ...
  static Future<CollectPlaylist> getAllCollectsResource(int mid) async {
    ...
    while (hasMore) {
      ...
      allMedias.addAll(AudioInfo.convertBiliMediaListToAudioInfoList(collectsResourceData.medias));
      ...
      if (hasMore) {
        ...
        collectsResourceDataInfo = collectsResourceData.info ?? CollectsDataDto();
      }
    }
  ...
  static Future<CollectsResourceData> _getCollectsResource(int mid,int pn) async {
    int ps = 20;//每页20条

    final collectsResource = await RhttpUtils().get(
        ApiConstants.resourceList,
        query: {
    ...
    return CollectsResource.fromJson(collectsResource.bodyToJson).data ?? CollectsResourceData();
  }

  //获取当前用户创建的所有合集信息
  static Future<CollectsListAllDto> getAllCreatedCollects2(String userId) async {
  ...
  static Future<CollectPlaylist> getSeasons(int seasonId) async {
    ...
    while (hasMore) {
      ...
      allMedias.addAll(AudioInfo.convertBiliSeasonResponseArchivesToAudioInfoList(collectsResourceData.archives));

      if(collectsResourceData.page!.total! - ps * pn<=0 ){
        final seasonResponseMeta = collectsResourceData.meta;
        hasMore = false;
        final user = await BiliUserApi.getCollects2Resource(seasonResponseMeta!.mid!.toString());
        upperDto = CollectsUpperDto(
          mid: seasonResponseMeta.mid,
          name: user.name,
          face: user.face
        );
  ...
  static Future<List<AudioInfo>> getSeasonsSample(int seasonId) async {
    ...
    final collectsResourceData = await _getSeasons(seasonId, 1,Constants.maxOfPreviewAudios);
  ...
  static Future<BiliSeasonResponseData> _getSeasons(int seasonId,int pn,int ps) async {

    final collectsResource = await RhttpUtils().get(
        ApiConstants.getSeasons,
        query: {
    ...
    return BiliSeasonResponse.fromJson(collectsResource.bodyToJson).data ?? BiliSeasonResponseData();
  }
  ...
  static Future<BiliSeriesResponseDtoData> _getAllSeries(String uid,int seriesId,int pn,int ps) async {
  ...
  return BiliSeriesResponseDto.fromJson(collectsResource.bodyToJson).data ?? BiliSeriesResponseDtoData();
}
