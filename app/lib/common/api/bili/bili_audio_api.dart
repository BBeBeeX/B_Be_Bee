...
class BiliAudioApi {
  static Future<List<AudioToplistPeriodItem>> getAudioTopList() async {
    final audioTopListResponse = await RhttpUtils.instance
        .get(ApiConstants.audioToplist, query: {'list_type': '1'});
    final data =
        AudioToplistResponse.fromJson(audioTopListResponse.bodyToJson).data;
  ...
  static Future<AudioTopMusicData> getAudioTopMusicList(String listId) async {
    final audioTopMusicListResponse = await RhttpUtils.instance
        .get(ApiConstants.audioTopMusicList, query: {'list_id': listId});
    return AudioTopMusicListResponse.fromJson(
                audioTopMusicListResponse.bodyToJson)
            .data ??
        AudioTopMusicData();
  }
}
